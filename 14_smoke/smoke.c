...
int main() {
	...
	digitalWrite(PIN_BUZZ, HIGH); // switch off the buzzer by default
	...
	while (1) {
		...
		digit_val = digitalRead(PIN_SMOKE);
		if (digit_val != current_val) {
			printf("Smoke value changed: %d -> %d\n", current_val, digit_val);
			current_val = digit_val;
		}
